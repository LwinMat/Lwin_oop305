...
int main(int argc, char** argv)
{
	...
	if (argc == 5) {
		...
		std::string stringLibrary{};
		size_t cntSize = 0u;

		while (readFile && cntSize < 4) {
			std::getline(readFile, stringLibrary);

			if (readFile.good() && stringLibrary[0] != '#') {
				Book temp(stringLibrary);
				library += temp;
				cntSize += 1;
			}
		}
		...
		while (std::getline(readFile, stringLibrary)) {
	...
	auto fixPriceOfBook = [usdToCadRate, gbpToCadRate](sdds::Book& b) {
		if (b.country() == "US") {
	...
	if (argc > 2) {
		...
		std::ifstream readMovie(argv[2]);

		std::string M_line = " ";
		size_t size = 0u;
		while (size <= 5 && std::getline(readMovie, M_line)) {
